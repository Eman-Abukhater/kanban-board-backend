...
/** ===================== AUTH (public) ===================== */
// Dev mint: admin/employee
app.post('/auth/dev-login', async (req, res) => {
  const { role = 'employee', user_id = 301 } = req.body || {};
  let name = 'User';
  try {
  ...
  res.json({ token, role: role === 'admin' ? 'admin' : 'employee', user_id: Number(user_id), name });
});

// Public viewer for a board
app.get('/auth/public-token/:fkboardid', (req, res) => {
  const { fkboardid } = req.params;
  const token = signViewerToken({ fkboardid });
  res.json({ token, role: 'viewer', fkboardid });
});

/** Attach auth (soft) for the rest — sets req.user if token present */
app.use(authMiddleware);

/** ===================== MEMBERS/PROJECTS (open) ===================== */
...
app.post('/boards', requireRole('admin'), async (req, res) => {
  const { projectName, fkpoid, addedbyid, addedby, description, memberIds } =
    req.body || {};
  if (!projectName || !fkpoid)
    return res
      .status(400)
      .json({ error: 'projectName and fkpoid are required' });
  ...
  try {
    ...
    const row = {
      ...
      members: (withMembers.boardMembers || []).map((m) => ({
        id: m.user_id,
        name: m.user.name,
      })),
...
app.patch('/boards/:boardid', requireRole('admin'), async (req, res) => {
...
app.delete('/boards/:boardid', requireRole('admin'), async (req, res) => {
  const boardid = Number(req.params.boardid);
  try {
...
// viewer allowed for this board; others open
app.get('/boards/:fkboardid/kanban', allowViewerForBoard, async (req, res) => {
  const { fkboardid } = req.params;
  try {
    ...
    res.json({
      lists,
      board: { title: board.title, fkboardid: board.fkboardid, status: board.status },
      progress,
    });
  } catch (e) {
    console.error(e);
...
// add list (admin)
app.post('/boards/:fkboardid/lists', requireRole('admin'), async (req, res) => {
  const { fkboardid } = req.params;
...
// delete list (admin)
app.delete('/lists/:listId', requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const lists = await prisma.list.findMany({
      where: { board_id: deleted.board_id },
      orderBy: { position: 'asc' },
    });
...
// add card (admin)
app.post('/lists/:listId/cards', requireRole('admin'), async (req, res) => {
  const { listId } = req.params;
  ...
  try {
    ...
    const created = await prisma.card.create({
      data: { list_id: listId, title: String(title), position: count },
    });
    res.json({
...
// delete card (admin)
app.delete('/cards/:cardId', requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const cards = await prisma.card.findMany({
      where: { list_id: deleted.list_id },
      orderBy: { position: 'asc' },
    });
    for (let i = 0; i < cards.length; i++) {
      if (cards[i].position !== i) {
        await prisma.card.update({
          where: { card_id: cards[i].card_id },
          data: { position: i },
        });
      }
    }
...
// reorder lists (admin)
app.patch('/lists/reorder', requireRole('admin'), async (req, res) => {
  const { board_id, from_list_id, to_list_id } = req.body || {};
  try {
    const lists = await prisma.list.findMany({
      where: { board_id },
      orderBy: { position: 'asc' },
...
// move card (admin)
app.patch('/cards/move', requireRole('admin'), async (req, res) => {
  const { card_id, source_list_id, dest_list_id } = req.body || {};
  try {
    await prisma.card.update({
      where: { card_id },
      data: { list_id: dest_list_id },
    });

    const src = await prisma.card.findMany({
      where: { list_id: source_list_id },
      orderBy: { position: 'asc' },
    });
    for (let i = 0; i < src.length; i++) {
      await prisma.card.update({
        where: { card_id: src[i].card_id },
        data: { position: i },
      });
    }
    const dst = await prisma.card.findMany({
      where: { list_id: dest_list_id },
      orderBy: { position: 'asc' },
    });
    for (let i = 0; i < dst.length; i++) {
      await prisma.card.update({
        where: { card_id: dst[i].card_id },
        data: { position: i },
      });
    }
    res.json({ ok: true });
...
/** Edit card — allow admin; allow employee if assigned to ANY task on this card */
...
app.put('/cards/:cardId', uploadAny, async (req, res) => {
  ...
  try {
    ...
    // Permission: admin OR employee assigned to any task under this card
    const u = req.user;
    if (!u || u.typ === 'viewer') {
    ...
    if (u.role !== 'admin') {
      const myTasks = await prisma.task.count({
        where: { card_id: cardId, assigned_to: Number(u.sub) || -1 },
      });
      if (myTasks === 0) {
    ...
    if (file) {
      ...
      if (current.image_url?.startsWith('/uploads/')) {
        const oldFsPath = path.join(
          UPLOAD_DIR,
          path.basename(current.image_url)
        );
...
// Add task (admin only)
app.post('/cards/:cardId/tasks', requireRole('admin'), async (req, res) => {
  const { cardId } = req.params;
  ...
  try {
    const created = await prisma.task.create({
      data: {
        ...
        status: 'todo',
        assigned_to: Number.isFinite(Number(assigneeId))
          ? Number(assigneeId)
          : null,
      },
    });
...
// Complete/uncomplete task (admin OR the assignee)
app.patch('/tasks/:taskId', async (req, res) => {
  const u = req.user;
  if (!u || u.typ === 'viewer') return res.status(401).json({ error: 'auth required' });

  const { taskId } = req.params;
  const { completed } = req.body || {};
  try {
    const t = await prisma.task.findUnique({ where: { task_id: taskId } });
    if (!t) return res.status(404).json({ error: 'task not found' });

    if (u.role !== 'admin' && Number(u.sub) !== Number(t.assigned_to)) {
      return res.status(403).json({ error: 'forbidden' });
    }
...
app.delete('/tasks/:taskId', requireRole('admin'), async (req, res) => {
  const { taskId } = req.params;
  try {
...
// Add tag (admin only)
app.post('/cards/:cardId/tags', requireRole('admin'), async (req, res) => {
  const { cardId } = req.params;
...
app.delete('/tags/:tagId', requireRole('admin'), async (req, res) => {
...
// Add comment (any token incl. viewer; or anonymous)
app.post('/cards/:cardId/comments', async (req, res) => {
  ...
  try {
    const nameFromToken = req.user?.name || null;
    const created = await prisma.comment.create({
      data: {
        card_id: cardId,
        author: String(author || nameFromToken || 'Anonymous'),
...
app.patch('/boards/:fkboardid/close', requireRole('admin'), async (req, res) => {
  const { fkboardid } = req.params;
  try {
    const board = await prisma.board.findUnique({
      where: { fkboardid },
      include: { lists: { include: { cards: { include: { tasks: true } } } } },
    });
    if (!board) return res.status(404).json({ error: 'board not found' });
    ...
    const progress = totalCards
      ? Math.round(((doneList?.cards.length || 0) / totalCards) * 100)
      : 0;
    if (progress < 100) {
...
/** Health */
app.get('/', (_req, res) => res.send('kanban backend OK'));
